...
(() => {
  ...
  function coverageText(snapshot) {
    const labels={sampling:'快照采样',partial:'部分可见',unavailable:'暂不可用',not_connected:'未接入',application_log:'应用日志（部分）',waiting:'等待日志'};
    const coverage=snapshot?.coverage||{};
    return [['进程','processes'],['TCP','network'],['打开文件','open_files'],['工具 / MCP','tool_calls']].map(([name,key])=>`${translated(name)}: ${translated(labels[coverage[key]]||'未接入')}`).join(' · ')+' · '+translated('实际文件读写 / 浏览器内 AI：未接入');
  }
  ...
  function renderTimeline(){const a=auditState.current;if(!a)return;const mapping={AGENT:['agent_trace'],TOOLS:['tool_call','mcp','api'],PROCESS:['process'],FILES:['filesystem'],NETWORK:['network','browser']},items=[];
    ...
    if(auditState.filter!=='SELF REPORT')for(const e of a.events){
      ...
      const labels={process_observed:'发现进程',process_disappeared:'进程不再可见',connection_observed:'发现 TCP 连接',connection_disappeared:'连接不再可见',open_file_observed:'打开文件观察',application_tool_request:'应用记录工具请求',application_tool_result:'应用记录工具结果'};
      const attribution=translated(e.attribution_method==='parent_process'?'子进程关联':'软件进程匹配');
      if(auditState.filter==='ALL'||(mapping[auditState.filter]||[]).includes(e.source_type)||auditState.filter==='VIOLATIONS'&&evaluation?.decision==='violation')items.push({at:e.timestamp,order:1,html:`<article class="agent-list-row"><time>${formatTime(e.timestamp)}</time><div><b>${esc(labels[e.command_category]||`${e.source_type} · ${e.action_type}`)}</b><p>${esc(e.resource)}</p><small>${esc(e.actor)}${e.process_id!=null?` · PID ${e.process_id} · ${esc(attribution)}`:''} · ${esc(translated(e.source_name))} · ${esc(translated(e.independent?'独立记录':e.attribution_method==='application_log'?'应用日志 · 非独立证据':'Agent 材料'))}</small></div><span class="state-badge ${evaluation?.decision==='violation'&&e.status!=='blocked'?'agent-alert':''}">${e.status==='blocked'?'已阻止':evaluation?.decision==='violation'?'异常':evaluation?.decision==='allowed'?'规则内':e.attribution_method==='application_log'?'应用记录':e.status==='observed'?'状态观察':'待评估'}</span></article>`});
